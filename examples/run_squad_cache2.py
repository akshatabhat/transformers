...
def compute_cache(args, train_dataset, bert_model):
    ...
    print("Number of examples:", len(cached_all_outputs))
    #logger.info("Size of input_ids cache: %f", sys.getsizeof(cached_all_input_ids[0].storage()))
    #logger.info("Size of bert_out0 cache: %f", sys.getsizeof(cached_all_outputs[0].storage()))
    #logger.info("Size of bert_out cache: %f", sys.getsizeof(cached_all_outputs[0].storage())*len(cached_all_outputs))
...
def train(args, train_dataset, model, tokenizer):
    ...
    for i in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            model.train()
            batch = tuple(t.to(args.device) for t in batch)
            bert_out = torch.Tensor(batch[0].size(0), 50, 768).to(args.device)
            print("bert_out:",bert_out)
            qa_inputs = {'start_positions': batch[3],
                    'end_positions':   batch[4],
                    'bert_last_hidden': bert_out}
            #print(qa_inputs['bert_last_hidden'])
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    all_cached_outputs = torch.zeros(len(all_input_ids), dtype=torch.float)     # dummy tensor to store cached outputs
    if evaluate:
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                ...
                                all_cls_index, all_p_mask, all_cached_outputs)
...
if __name__ == "__main__":
    main()
stilBertConfig, DistilBertForQuestionAnswering, DistilBertTokenizer,
                            AlbertConfig, AlbertForQuestionAnswering, AlbertTokenizer)
